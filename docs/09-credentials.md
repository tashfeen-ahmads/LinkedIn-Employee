...
## 6. Google — sign-in only

Google is used for **one** thing: signing in. The Calendar integration described
here previously was never built — the worker has no `/auth/google/link` and no
`/auth/google/callback`, so the button that pointed at them did nothing, and the
runbook told you to register a redirect URI for a route that does not exist.
That button and the matching onboarding step have been removed.
...
2. **Credentials** → Create OAuth client ID → Web application → add the redirect
   URI above.

No Calendar API, no sensitive scopes, so no Google verification review and no
"unverified app" interstitial. Add your rep as a test user and leave the consent
screen in Testing.

Until Calendar is built, meetings are booked by hand. That is the behaviour the
Reply Agent was written for anyway: with no calendar connected it offers to send
times rather than proposing any, because the model never invents a datetime.
