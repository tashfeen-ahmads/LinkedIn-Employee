...
const MUTATIONS = [
  ...
  },
  {
    ...
    file: "apps/worker/src/server.ts",
    from: "    const reconciled = await reconcileAccount(ctx, parsed.data.workspaceId, parsed.data.userId, mine);",
    to: "    const reconciled = await reconcileAccount(ctx, parsed.data.workspaceId, parsed.data.userId, accounts);",
    pkg: "@le/worker",
  },
  {
  ...
  },
  {
    ...
    file: "packages/linkedin/src/unipile.ts",
    from: "      location: resolved.locations.length ? resolved.locations : undefined,",
    to: "      location: query.geographies?.length ? query.geographies : undefined,",
    pkg: "@le/linkedin",
  },
  {
  ...
  },
  {
    ...
    rule: "Several titles or keywords are searched as alternatives; space-joined they are an AND that matches nobody",
    file: "packages/linkedin/src/unipile.ts",
    from: '  return quoted.length === 1 ? quoted[0] : quoted.join(" OR ");',
    to: '  return quoted.join(" ");',
    pkg: "@le/linkedin",
  },
  {
