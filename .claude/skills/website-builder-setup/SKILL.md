...
Added by the team, not by the upstream skill.

The install steps above assume a laptop. This repository is worked on from
Claude Code on the web, where each session runs in a container that is thrown
away afterwards — so `npm install -g`, and anything written to `~/.claude.json`,
is gone by the next session. Only what is committed here survives.

So the pieces live in the repository instead:

- **Framer Motion** is a dependency of `apps/web`, not a global install.
- **21st.dev Magic** is declared in `.mcp.json` at the repository root and reads
  its key from `TWENTY_FIRST_API_KEY` in the environment. The key is not in the
  file and must not be put there.
- **UI/UX Pro Max** is not installed. `npm install -g uipro-cli` and
  `npx uipro-cli init` are both refused by the sandbox's permission rules. Run
  `uipro init --ai claude` on a laptop, in this repository, and commit whatever
  it writes under `.claude/`; then it is here for everyone.

The design system the app is actually built on lives in
`apps/web/src/app/globals.css`. Read it before adding a component: it has
tokens for colour, type, spacing and elevation, and a component should be built
from those rather than from an inline `style` attribute.
